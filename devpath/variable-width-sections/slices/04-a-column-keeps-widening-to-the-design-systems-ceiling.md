...
- [ ] `.rstk-nav-sections`'s `gap` and `padding` — both `var(--slds-g-spacing-4, 1rem)` — are pinned by no test anywhere, and two things this spec asserts rest on them. `## Traps`'s first entry names the container's padding as what keeps a section card's `box-shadow` from clipping vertically inside the scroll container ("unless the container carries padding, which it does"), and this slice's criterion 3 asserts the ~1,072px scroll threshold, whose arithmetic is 6×160 + 5×16 gaps + 2×16 padding — so deleting either declaration silently falsifies both, and every test stays green. The grid pin already reads the whole `.rstk-nav-sections` rule body as text and asserts five of its seven declarations (`display`, `grid-template-columns`, `grid-auto-flow`, `justify-content`, `overflow-x`); `gap` and `padding` are the two it skips, and each is one `toContain` in the idiom already there. Pre-existing — it arrived with slice 01's pin and nothing in `a6a6a6c`, `2eef6a5` or `18a9e68` touched it, and the shipped declarations are both correct, so this is a coverage gap rather than a defect
